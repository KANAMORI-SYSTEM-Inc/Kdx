...
- このプロジェクトでは、Supabaseから設計内容、アクチュエータの動作を取得し、それをPLCで動作するCSVファイル形式のニモニックプログラムに変換することを目的としています
- KdxProjectsというsupabaseのスキーマを定義したクラスファイルがあり、それを使ってデータベースとの通信を実装しています。
...
- Viewを追加する際は既存機能が無いか調べて、共通コンポーネント化してください。