...
namespace KdxDesigner.ViewModels.Settings
{
    ...
    public partial class MemorySettingViewModel : ObservableObject
    {
        ...
        {
            // プロファイル入力ダイアログを表示
            var dialog = new ViewsMemory.ProfileInputDialog
            {
                DialogTitle = "PLC用プロファイル作成",
                Owner = Application.Current.Windows.OfType<ViewsMemory.MemorySettingWindow>().FirstOrDefault()
            };
        ...
        private async Task ExecuteMemorySetting()
        {
            ...
            var progressWindow = new ViewsMemory.MemoryProgressWindow
            {
