...
namespace KdxDesigner.Controls
{
    ...
    public partial class ProcessDetailListControl : UserControl
    {
        ...
        /// 変更を保存
        /// </summary>
        private async void SaveProcessDetails_Click(object sender, RoutedEventArgs e)
        {
            if (ProcessDetails == null || Repository == null)
            {
            ...
            {
                // 編集をコミット
                ProcessDetailGrid.CommitEdit(DataGridEditingUnit.Row, true);

                // 全ての工程詳細を保存
                foreach (var detail in ProcessDetails)
                {
                    await Repository.UpdateProcessDetailAsync(detail);
                }
